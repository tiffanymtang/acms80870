...
In this document, we repeat the analysis from 02_modeling.Rmd but with unsure pixels now classified as "Clouds".
</div>

```{r}
cloud_data_orig <- load_cloud_data()
cloud_data_ls <- clean_cloud_data(cloud_data_orig, unsure_class = "Clouds")
...
new_mean_valid_auprc_df <- valid_auprc_df |> 
  dplyr::summarise(dplyr::across(-fold, ~ mean(.x, na.rm = TRUE)))
...
new_auroc_df <- list(
  "Raw Features Only" = auroc_df,
  "Including Engineered Features" = new_auroc_df
) |> 
...
new_auprc_df <- list(
  "Raw Features Only" = auprc_df,
  "Including Engineered Features" = new_auprc_df
) |> 
  dplyr::bind_rows(.id = "Feature Set")
